...
class HallucinationSample:
    ...
    def from_json(cls, json_dict: dict) -> "HallucinationSample":
        ...
        if "prompt" not in json_dict:
            # 假设旧格式包含 query 和 context
            query = json_dict.get("query", "")
            context = json_dict.get("context", "")
            json_dict["prompt"] = f"Question: {query}\nContext: {context}"
            json_dict["answer"] = json_dict.pop("output", "")
            json_dict["labels"] = json_dict.pop("hallucination_labels", [])
            # 为缺失的字段提供默认值
            json_dict.setdefault("split", "train")
            json_dict.setdefault("task_type", "qa")
            json_dict.setdefault("dataset", "ragtruth")
            json_dict.setdefault("language", "en")
        return cls(
